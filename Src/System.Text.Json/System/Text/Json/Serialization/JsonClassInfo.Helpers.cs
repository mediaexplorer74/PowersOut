...
namespace System.Text.Json
{
    internal partial class JsonClassInfo
    {
            ...
            JsonSerializerOptions options)
        {
            ...
            if (!(typeof(IEnumerable).GetTypeInfo().IsAssignableFrom(/*queryType*/default)) ||
                queryType == typeof(string) ||
                queryType.GetTypeInfo().IsInterface ||
                queryType.IsArray ||
            ...
            {
                if (candidate.GetTypeInfo().IsAssignableFrom(/*queryType*/default))
                {
                    return candidate;
                }
            }
        ...
        {
            return type.GetTypeInfo().IsGenericType && type.GetGenericTypeDefinition() == typeof(ISet<>);
        }
        ...
        public static bool HasConstructorThatTakesGenericIEnumerable(Type type, JsonSerializerOptions options)
        {
            ...
            return default;//type.GetConstructor(new Type[] { typeof(List<>).MakeGenericType(elementType) }) != null;
        }
        ...
        public static bool IsNativelySupportedCollection(Type queryType)
        {
            ...
            if (queryType.GetTypeInfo().IsGenericType)
            {
                return s_nativelySupportedGenericCollections.Contains(
                    queryType.GetGenericTypeDefinition().FullName);
            }
        ...
        {
            Type bestMatch = null;
            Type[] interfaces = default;//queryType.GetInterfaces();
