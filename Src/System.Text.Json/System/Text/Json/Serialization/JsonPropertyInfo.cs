...
namespace System.Text.Json
{
    ...
    internal abstract class JsonPropertyInfo
    {
                ...
                if (HasGetter)
                {
                    ...
                    if (HasSetter)
                    {
                        ...
                        else if (ClassType == ClassType.Enumerable)
                        {
                            ...
                            if (DeclaredPropertyType != ImplementedPropertyType &&
                                (!typeof(IList).GetTypeInfo().IsAssignableFrom(/*RuntimePropertyType*/default) ||
                            ...
                            else if (JsonClassInfo.IsDeserializedByConstructingWithIList(/*RuntimePropertyType*/default) ||
                                (!typeof(IList).GetTypeInfo().IsAssignableFrom(/*RuntimePropertyType*/default) &&
                            ...
                            else if (RuntimePropertyType.GetTypeInfo().IsGenericType &&
                                RuntimePropertyType.FullName.StartsWith(
                                    JsonClassInfo.ImmutableNamespaceName) &&
                                RuntimePropertyType.GetTypeInfo().GenericTypeArguments.Length == 1)
                            {
            ...
            JsonSerializerOptions options)
        {
            ...
            Options = options;
            IsNullableType = runtimePropertyType.GetTypeInfo().IsGenericType 
                && runtimePropertyType.GetGenericTypeDefinition() == typeof(Nullable<>);
            CanBeNull = IsNullableType || !runtimePropertyType.GetTypeInfo().IsValueType;

            if (converter != null)
            {
                ConverterBase = converter;

                HasInternalConverter = (converter.GetType().GetTypeInfo().Assembly == GetType().GetTypeInfo().Assembly);
