    ...
    public sealed partial class JsonSerializerOptions
    {
        ...
        public JsonConverter GetConverter(Type typeToConvert)
        {
            ...
            if (converter != null)
            {
                ...
                if (!converterTypeToConvert.GetTypeInfo().IsAssignableFrom(/*typeToConvert*/default) &&
                    !typeToConvert.GetTypeInfo().IsAssignableFrom(/*converterTypeToConvert*/default))
                {
        ...
        private JsonConverter GetConverterFromAttribute(JsonConverterAttribute converterAttribute, Type typeToConvert, Type classTypeAttributeIsOn, PropertyInfo propertyInfo)
        {
            ...
            {
                ConstructorInfo ctor = default;//type.GetTypeInfo().GetConstructor(Type.EmptyTypes);
                if (!typeof(JsonConverter).GetTypeInfo().IsAssignableFrom(/*type*/default) || !ctor.IsPublic)
                {
                    ThrowHelper.ThrowInvalidOperationException_SerializationConverterOnAttributeInvalid(
                        classTypeAttributeIsOn, propertyInfo);
                }
        ...
        private static Attribute GetAttributeThatCanHaveMultiple(Type classType, Type attributeType, PropertyInfo propertyInfo)
        {
            //FIXIT
            //object[] 
            var attributes = propertyInfo?.GetCustomAttributes(/*attributeType*/default, inherit: false);
            return GetAttributeThatCanHaveMultiple(attributeType, classType, propertyInfo, /*attributes*/default);
        }
        ...
        private static Attribute GetAttributeThatCanHaveMultiple(Type classType, Type attributeType)
        {
            //FIXIT
            //object[] 
            var attributes = classType.GetTypeInfo().GetCustomAttributes(/*attributeType*/default, inherit: false);
            return GetAttributeThatCanHaveMultiple(attributeType, classType, null, /*attributes*/default);
        }
