    ...
    internal sealed partial class JsonClassInfo
    {
        ...
        public JsonClassInfo(Type type, JsonSerializerOptions options)
        {
            ...
            switch (ClassType)
            {
                case ClassType.Object:
                    {
                        PropertyInfo[] properties = default;//type.GetTypeInfo().GetDeclaredProperty("public"/*BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);//GetProperties(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic*/);
        ...
        private bool DetermineExtensionDataProperty(Dictionary<string, JsonPropertyInfo> cache)
        {
            ...
            if (jsonPropertyInfo != null)
            {
                ...
                if (!typeof(IDictionary<string, JsonElement>).GetTypeInfo()
                    .IsAssignableFrom(/*declaredPropertyType*/default) &&
        ...
        public static Type GetElementType(Type propertyType, Type parentType, MemberInfo memberInfo, JsonSerializerOptions options)
        {
            ...
            if (!typeof(IEnumerable).GetTypeInfo().IsAssignableFrom(/*implementedType*/default))
            {
                return null;
            }
            ...
            if (implementedType.GetTypeInfo().IsGenericType)
            {
                Type[] args = implementedType.GetTypeInfo().GenericTypeArguments;// Get//.GetGenericArguments();
                ...
                if ((classType == ClassType.Dictionary || classType == ClassType.IDictionaryConstructible) &&
                   ...
                   // args[0].GetTypeInfo().UnderlyingSystemType == typeof(string)
                   )
                {
                    return args[1];
                }
            ...
            //FIXIT
            if (implementedType.GetTypeInfo().IsAssignableFrom(/*typeof(IList)*/default) ||
                //implementedType.GetTypeInfo().IsAssignableFrom(typeof(IDictionary)) ||
        ...
        public static ClassType GetClassType(Type type, JsonSerializerOptions options)
        {
            ...
            if (implementedType.GetTypeInfo().IsGenericType 
                && implementedType.GetGenericTypeDefinition() == typeof(Nullable<>))
            {
            ...
            if (typeof(IDictionary).GetTypeInfo().IsAssignableFrom(/*implementedType*/default) 
                || IsDictionaryClassType(implementedType))
            {
