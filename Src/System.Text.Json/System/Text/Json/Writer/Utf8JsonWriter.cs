...
//#if BUILDING_INBOX_LIBRARY
//FIXIT
                    await _stream.WriteAsync(_arrayBufferWriter.WrittenMemory,                     
                    /*cancellationToken*/0,0).ConfigureAwait(false);
                    /*
#else
                    Debug.Assert(_arrayBufferWriter.WrittenMemory.Length == _arrayBufferWriter.WrittenCount);
                    bool result = MemoryMarshal.TryGetArray(_arrayBufferWriter.WrittenMemory, 
                        out ArraySegment<byte> underlyingBuffer);
                    Debug.Assert(underlyingBuffer.Offset == 0);
                    Debug.Assert(_arrayBufferWriter.WrittenCount == underlyingBuffer.Count);
                    await _stream.WriteAsync(underlyingBuffer.Array, underlyingBuffer.Offset, 
                        underlyingBuffer.Count, cancellationToken).ConfigureAwait(false);
#endif
                    */
