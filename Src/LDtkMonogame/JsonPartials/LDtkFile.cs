...
public partial class LDtkFile
{
    ...
    {
        LDtkFile? file = JsonSerializer.Deserialize<LDtkFile>(
            File.ReadAllText(filePath)/*, Constants.JsonSourceGenerator.LDtkFile*/);
        if (file == null)
        {
    ...
    public LDtkWorld? LoadWorld(Guid iid)
    {
        foreach (LDtkWorld world in Worlds)
        {
            ...
            foreach (LDtkLevel level in world.Levels)
            {
                ...
                {
                   level.FilePath = Path.Combine(Path.GetDirectoryName(FilePath), level.ExternalRelPath);
                }
