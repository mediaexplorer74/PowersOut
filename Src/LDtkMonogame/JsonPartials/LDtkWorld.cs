...
public partial class LDtkWorld
{
    ...
    public LDtkLevel LoadLevel(LDtkLevel rawLevel)
    {
        ...
        else
        {
            ...
            if (Content != null)
            {
                level = LDtkLevel.FromFile(Path.Combine(Path.GetDirectoryName(FilePath), rawLevel.ExternalRelPath), Content);
            }
            else
            {
                level = LDtkLevel.FromFile(Path.Combine(Path.GetDirectoryName(FilePath), rawLevel.ExternalRelPath));
            }
