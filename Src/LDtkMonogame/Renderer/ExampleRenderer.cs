...
public class ExampleRenderer : IDisposable
{
    ...
    //     but perceptually the images should be identical. This call does not premultiply
    //     the image alpha, but areas of zero alpha will result in black color data.
    public static Texture2D FromFile2(GraphicsDevice graphicsDevice, string path)
    {
        if (path == null)
        {
    ...
    Texture2D GetTexture(LDtkLevel level, string? path)
    {
        ...
        if (content == null)
        {
            ...
            string assetName = Path.Combine(directory, path);
            tilemap = /*Texture2D.*/FromFile2(graphicsDevice, assetName);
        }
        else
        {
            string file = Path.ChangeExtension(path, null);
            string directory = Path.GetDirectoryName(level.WorldFilePath)!;
            string assetName = Path.Combine(directory, file);
